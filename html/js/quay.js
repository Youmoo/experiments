var Quay = function ($el) {
    ...
    //stores a normalized key to a handler value
    this.bindings = {}

    var pressing = [],//待处理的按钮事件
        special_keys = {
    ...
    /**
     * @param key_bindings 按键列表到事件处理器的映射
     */
    this.capture = function (key_bindings) {
        ...
        for (var i = 0; i < keys.length; i++) {
            var key = keys[i],//比如'ctrl_a,alt_b'
                keys_split = key.split(',');//比如['ctrl_a','alt_b']
